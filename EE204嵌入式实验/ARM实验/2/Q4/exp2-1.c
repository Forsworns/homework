...
uint8_t i = 0;	
uint32_t times = 0;
uint32_t digitTimes=0;
uint32_t USR_SWtimes = 0;
...
int main(void)
{
  ui32SysClock = SysCtlClockFreqSet((SYSCTL_XTAL_16MHZ |SYSCTL_OSC_INT |SYSCTL_USE_OSC), 1600000);		
	S800_GPIO_Init();
	S800_I2C0_Init();
	S800_SYSTICK_Init();
	while (1) {
		now = GPIOPinRead(GPIO_PORTJ_BASE,GPIO_PIN_0);
		if(prev!=now && now==PRESSED){
			SysTickDisable();
			GPIOPinWrite(GPIO_PORTN_BASE,GPIO_PIN_0,GPIO_PIN_0);
			switch(USR_SWtimes++%4){
				case 0: {
					deltaTime = ui32SysClock; 
					break;
				}
				case 1: {
					deltaTime = ui32SysClock*2; 
					break;
				}
				case 2: {
					deltaTime = ui32SysClock/5; 
					break;
				}
				case 3: {
					deltaTime = ui32SysClock/2; 
			...
			SysTickPeriodSet(deltaTime);
			while(SysTickPeriodGet()!=deltaTime);
...
void S800_SYSTICK_Init(void)
{
	...
	SysTickEnable();
	SysTickIntEnable();
	SysTickIntRegister(SysTick_Handler);//register ISR必须在enable INT之后
}
...
{
	digitTimes++;
	I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2, DIGIT[digitTimes%8]);			// Select digit
	I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,seg7[digitTimes%8]);		// Display number			
	I2C0_WriteByte(PCA9557_I2CADDR,PCA9557_OUTPUT,~DIGIT[digitTimes%8]);
}
