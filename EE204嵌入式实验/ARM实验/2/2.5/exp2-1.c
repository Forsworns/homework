...
void S800_GPIO_Init(void)
{
	...
	IntPrioritySet(INT_GPIOJ_TM4C129, 0);
	IntPrioritySet(15, 0x20);
	IntEnable(INT_GPIOJ_TM4C129); //先激活PORTJ才能激活特定的引脚
...
void S800_SYSTICK_Init(void)
{
	SysTickPeriodSet(ui32SysClock/10);
	SysTickEnable();
	SysTickIntEnable();
...
uint8_t I2C0_ReadByte(uint8_t DevAddr, uint8_t RegAddr)
{
	...
	if (rop)
	{ // if there is error in the transmission, return the error status
		return rop;
	}
	Delay(1);
