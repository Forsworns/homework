...
uint8_t position[] = {0x80, 0x40, 0x20, 0x10, 0x08, 0x04, 0x02, 0x01};
...
void SysTick_Handler(void)
{
		...
		if (flag) {
				I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT2, position[i]);
				I2C0_WriteByte(TCA6424_I2CADDR,TCA6424_OUTPUT_PORT1,seg7[i+1]);				
				I2C0_WriteByte(PCA9557_I2CADDR,PCA9557_OUTPUT,~position[i]);
				i = (i + 1) % 8;
		}
		else {
				if (led) {
