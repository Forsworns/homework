...
int main(void)
{
  ...
  UARTIntEnable(UART0_BASE, UART_INT_RX | UART_INT_RT); //Enable UART0 RX,TX interrupt
    //UARTIntEnable(UART0_BASE, UART_INT_RX);   //Enable UART0 RX,TX interrupt
  IntMasterEnable();        
...
void UART0_Handler(void)
{
  ...
  UARTIntClear(UART0_BASE, uart0_int_status);                               //Clear the asserted interrupts
    ...
    //UARTCharPutNonBlocking(UART0_BASE,UARTCharGetNonBlocking(UART0_BASE));
        ...
        s2[i] = UARTCharGetNonBlocking(UART0_BASE);
        i=i+1;
        Delay(10000);
    } */
    
    get_command_from_host();
    if (strcmp(received_command, "AT+CLASS")==0) UARTStringPut("CLASS F1603301");
    else if (strcmp(received_command, "AT+STUDENTCODE")==0) UARTStringPut("CODE 516030910147");
...
void get_command_from_host() {
    ...
    while (UARTCharsAvail(UART0_BASE)) {
        received_command[command_ptr] = UARTCharGetNonBlocking(UART0_BASE);
        if ('a' <= received_command[command_ptr] && received_command[command_ptr] <='z')
            received_command[command_ptr] = 'A' + received_command[command_ptr] - 'a';
        ++command_ptr;
        GPIOPinWrite(GPIO_PORTN_BASE, GPIO_PIN_1, GPIO_PIN_1);
        Delay(10000);
    }
    received_command[command_ptr] = '\0';
}
