...
void UARTSend(char *target)
{
	while (*target != '\0') {
		UARTCharPut(UART0_BASE, *target++);
	}
}
  ...
  SysTickPeriodSet(ui32SysClock/SYSTICK_FREQUENCY);
   ...
   IntMasterEnable();		
	...
	while (1)
	{
		/*
		if (commandFlag) {
			if (strcmp(command, "AT+CLASS") == 0) {
				UARTSend("F1603305");
			}
			else if (strcmp(command, "AT+STUDENTCODE") == 0) {
				UARTSend("516021910746");
			}
			commandFlag = 0;
		}
		*/
		
		if (strcmp(command, "AT+CLASS") == 0 || strcmp(command, "at+class") == 0) {
			UARTSend("CLASSF1603305");
			commandClear();
		}
		else if (strcmp(command, "AT+STUDENTCODE") == 0 || strcmp(command, "at+studentcode") == 0) {
			UARTSend("CODE516021910746");
			commandClear();
		}
		...
		if (UARTCharsAvail(UART0_BASE))
		{
			...
			commandInsert(uart_receive_char);
		}
