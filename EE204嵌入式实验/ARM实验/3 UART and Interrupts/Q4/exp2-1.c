...
void UART0_Handler(void){
	...
	if(strlen(re_string)==11){
		sum1+=(re_string[0]-'0')*10*60;
		sum1+=(re_string[1]-'0')*60;
		sum1+=(re_string[3]-'0')*10;
		sum1+=(re_string[4]-'0');
		
		sum2+=(re_string[6]-'0')*10*60;
		sum2+=(re_string[7]-'0')*60;
		sum2+=(re_string[9]-'0')*10;
		sum2+=(re_string[10]-'0');
		...
		segM = segSS/6;
		segMM = segM/10;
		re_string[0] = segMM%6+'0';
		re_string[1] = segM%10+'0';
		re_string[2] = ':';
		re_string[3] = segSS%6+'0';
		re_string[4] = segS%10+'0';
		re_string[5] = '\0';
