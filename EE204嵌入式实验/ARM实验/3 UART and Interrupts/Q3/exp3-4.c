...
*/
int8_t isMonth(char *string){
	int8_t i;
	for(i = 0; i<12; ++i){
		if(strstr(string,MONTH[i])==string){
			break;
		}
		if(i == 11) return -1;
	}
...
void UART0_Handler(void)
{
  ...
  while(UARTCharsAvail(UART0_BASE))    											// Loop while there are characters in the receive FIFO.
  {
	...
	if(strlen(re_string)!=6) return;
	month = isMonth(re_string);
	if(month>=0 && month<=12){
		re_string[0] = MONTH[month][0];
		re_string[1] = MONTH[month][1];
		re_string[2] = MONTH[month][2];
		sum += (re_string[4]-'0')*10;
		sum += (re_string[5]-'0');
		if(re_string[3] == '+'){
			UARTStringPutNonBlocking(MONTH[(month+sum)%12]);
		}else{
			UARTStringPutNonBlocking(MONTH[(month-sum+12)%12]);
		}
	}else{
