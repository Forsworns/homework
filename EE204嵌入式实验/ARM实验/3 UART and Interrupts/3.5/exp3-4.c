...
char re_char;
char re_string[80]={'\0'};
const char RE_CLASS[] = "AT+CLASS";
const char RE_STUDENT[] = "AT+STUDENTCODE";
const char TR_CLASS[] = "CLASS F1603305";
const char TR_STUDENT[] = "CODE 516021910233";
int main(void)
{
  ...
  IntMasterEnable();		
	...
	//re_string = &re_char+1;//malloc 没法用，这样虽然初始化了指针，但是具体情况未知，可能会覆盖什么东西,那现在用数组埃别用指针了?
	while (1)
	{
...
void UART0_Handler(void)
{
  ...
  while(UARTCharsAvail(UART0_BASE))    											// Loop while there are characters in the receive FIFO.
  {
	...
	if(strlen(re_string)==strlen(RE_CLASS) && strcmp(re_string,RE_CLASS) == 0){
		UARTStringPutNonBlocking(TR_CLASS);
	}else if(strlen(re_string)==strlen(RE_STUDENT) && strcmp(re_string,RE_STUDENT) == 0){
		UARTStringPutNonBlocking(TR_STUDENT);
	}else{
