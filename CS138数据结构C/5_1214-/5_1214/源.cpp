...
//int Height = 0; //非二叉树转化成二叉树的深度
int TrueHeight = 1;//真实深度，也就是左子树深度，右子树（其实是兄弟）不增加这个深度
int sheet[100000][3] = { 0 };
int createFlag[100000] = { 0 }; //减少递归减少堆栈溢出风险
...
private:
	...
	/*void setDepth(Node *t, int &height, int &heightnum)const { //前序遍历设置各个节点的深度,heightnum是总高度，number是倒数第二行的结点数目
		if (t != NULL) {
			height++; //下降一层
					  //cout << t->data << ' ';
					  //cout << Height << endl;
...
int main() {
	...
	largeTree = trees.visit(maxPosition); //最大树
