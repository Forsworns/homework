...
NS_LOG_COMPONENT_DEFINE ("HwRoutingProtocol");

namespace hw {

NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

const uint32_t RoutingProtocol::HW_PORT = 268;
...
struct DeferredRouteOutputTag : public Tag
{
  ...
  GetTypeId ()
  {
    static TypeId tid = TypeId ("ns3::hw::DeferredRouteOutputTag")
      .SetParent<Tag> ()
      .SetGroupName ("Hw")
...
{
  static TypeId tid = TypeId ("ns3::hw::RoutingProtocol")
...
// specify the stream number for the random number generator
int64_t
...
// query routing cache for an existing route, for an outbound packet. this lookup is used by protocol
Ptr<Ipv4Route>
  ...
  if (m_socketAddresses.empty ())
    {
      ...
      NS_LOG_LOGIC ("No hw interfaces");
      Ptr<Ipv4Route> route;
      return route;
    }
  // removedAddresses 和 rt没指定初始值，怎么进行的下面的查询----------------------------------------------------------------------------------------------
...
// queue the packet until we find a route
void
                             ...
                             ErrorCallback ecb)
{
  ...
  // hw is not a multicast routing protocol
  if (dst.IsMulticast ())
    {
      return false;
    }
  ...
  // LOCAL DELIVARY TO HW INTERFACES
...
RoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif) const
{
  ...
  // returned when HW does not have a route; this causes the packet
  // to be looped back and handled (cached) in RouteInput() method
  // while a route is found. However, connection-oriented protocols
  // like TCP need to create an endpoint four-tuple (src, src port,
  // dst, dst port) and create a pseudo-header for checksumming.  So,
  // HW needs to guess correctly what the eventual source address
  // will be.
  //
  // For single interface, single address nodes, this is not a problem.
  // When there are possibly multiple outgoing interfaces, the policy
  // implemented here is to pick the first available HW interface.
  ...
  NS_ASSERT_MSG (rt->GetSource () != Ipv4Address (), "Valid HW source address not found");
  rt->SetGateway (Ipv4Address ("127.0.0.1"));
  rt->SetOutputDevice (m_lo);
  return rt;
}

//receive and process hw control packet
void
RoutingProtocol::RecvHw (Ptr<Socket> socket)
{
  ...
  NS_LOG_FUNCTION (m_mainAddress << " received hw packet of size: " << packetSize
                                 << " and packet id: " << packet->GetUid ());
  uint32_t count = 0;
  for (; packetSize > 0; packetSize = packetSize - 12)
    {
      count = 0;
      HwHeader hwHeader, tempHwHeader;
      packet->RemoveHeader (hwHeader);          // Deserialize and remove the header from the internal buffer. 
      NS_LOG_DEBUG ("Processing new update for " << hwHeader.GetDst ());
      ...
      for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j
           != m_socketAddresses.end (); ++j)
        {
          ...
          if (hwHeader.GetDst () == interface.GetLocal ())
            {
              if (hwHeader.GetDstSeqno () % 2 == 1)// this dstSeqno represent the destination sequence, so if it is odd, can't from me;
                {
                  NS_LOG_DEBUG ("Sent Hw update back to the same Destination, "
                                "with infinite metric. Time left to send fwd update: "
                                << m_periodicUpdateTimer.GetDelayLeft ());
                  count++;
                }
              else                                // dstSeqno is even, it is from me
                {
      ...
      NS_LOG_DEBUG ("Received a HW packet from "
                    << sender << " to " << receiver << ". Details are: Destination: " << hwHeader.GetDst () << ", Seq No: "
                    << hwHeader.GetDstSeqno () << ", HopCount: " << hwHeader.GetHopCount ());
      RoutingTableEntry fwdTableEntry, advTableEntry; // forward table and adverse table
      EventId event;
      bool permanentTableVerifier = m_routingTable.LookupRoute (hwHeader.GetDst (),fwdTableEntry);
      if (permanentTableVerifier == false) // didn't find it
        {
          if (hwHeader.GetDstSeqno () % 2 != 1)
            {
              NS_LOG_DEBUG ("Received New Route!");
              RoutingTableEntry newEntry (
                /*device=*/ dev, /*dst=*/
                hwHeader.GetDst (), /*seqno=*/
                hwHeader.GetDstSeqno (),
                /*iface=*/ m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver), 0),
                /*hops=*/ hwHeader.GetHopCount (), /*next hop=*/                          // next hop = sender, send back to the sender
                sender, /*lifetime=*/
        ...
        {
          if (!m_advRoutingTable.LookupRoute (hwHeader.GetDst (),advTableEntry))
            {
              ...
              m_advRoutingTable.LookupRoute (hwHeader.GetDst (),advTableEntry);
            }
          if (hwHeader.GetDstSeqno () % 2 != 1)
            {
              if (hwHeader.GetDstSeqno () > advTableEntry.GetSeqNo ())
                {
                  // Received update with better seq number. Clear any old events that are running (the larger seq number is, the better it is
                  if (m_advRoutingTable.ForceDeleteIpv4Event (hwHeader.GetDst ()))
                    {
                  ...
                  if (hwHeader.GetHopCount () != advTableEntry.GetHop ())
                    {
                      advTableEntry.SetSeqNo (hwHeader.GetDstSeqno ());
                      ...
                      advTableEntry.SetHop (hwHeader.GetHopCount ());
                      NS_LOG_DEBUG ("Received update with better sequence number and changed metric.Waiting for WST");
                      Time tempSettlingtime = GetSettlingTime (hwHeader.GetDst ());
                      ...
                      m_advRoutingTable.AddIpv4Event (hwHeader.GetDst (),event);
              ...
              else if (hwHeader.GetDstSeqno () == advTableEntry.GetSeqNo ())
                {
                  if (hwHeader.GetHopCount () < advTableEntry.GetHop ())
                    {
                      /*Received update with same seq number and better hop count.(the less hop number is, the better it is)
                      ...
                      m_advRoutingTable.ForceDeleteIpv4Event (hwHeader.GetDst ());
                      advTableEntry.SetSeqNo (hwHeader.GetDstSeqno ());
                      advTableEntry.SetLifeTime (Simulator::Now ());
                      advTableEntry.SetFlag (VALID);
                      advTableEntry.SetEntriesChanged (true);
                  ...
                  else
                    {
                       ...
                       * Discard that update.
                       */
                      if (!m_advRoutingTable.AnyRunningEvent (hwHeader.GetDst ()))
                        {
                      ...
                      NS_LOG_DEBUG ("Received update with same seq number and "
                                    "same/worst metric for, " << hwHeader.GetDst () << ". Discarding the update.");
                    }
                }
              else
                {
                  ...
                  if (!m_advRoutingTable.AnyRunningEvent (hwHeader.GetDst ()))
                    {
                      m_advRoutingTable.DeleteRoute (hwHeader.GetDst ());
                    }
                  NS_LOG_DEBUG (hwHeader.GetDst () << " : Received update with old seq number. Discarding the update.");
                }
            }
          else // seq number is odd
            {
              NS_LOG_DEBUG ("Route with infinite metric received for "
                            << hwHeader.GetDst () << " from " << sender);
              // Delete route only if update was received from my nexthop neighbor (because infinite means broken route)
              if (sender == advTableEntry.GetNextHop ())
                {
                  ...
                  m_routingTable.GetListOfDestinationWithNextHop (hwHeader.GetDst (),dstsWithNextHopSrc);
                  m_routingTable.DeleteRoute (hwHeader.GetDst ());
                  advTableEntry.SetSeqNo (hwHeader.GetDstSeqno ());
                  advTableEntry.SetEntriesChanged (true);
                  m_advRoutingTable.Update (advTableEntry);
      ...
      for (std::map<Ipv4Address, RoutingTableEntry>::const_iterator i = allRoutes.begin (); i != allRoutes.end (); ++i)
        {
          ...
          if ((i->second.GetEntriesChanged () == true) && (!m_advRoutingTable.AnyRunningEvent (temp.GetDestination ())))
            {
              hwHeader.SetDst (i->second.GetDestination ());
              hwHeader.SetDstSeqno (i->second.GetSeqNo ());
              hwHeader.SetHopCount (i->second.GetHop () + 1);
              temp.SetFlag (VALID);
      ...
      if (packet->GetSize () >= 12)
        {
          ...
          hwHeader.SetDst (m_ipv4->GetAddress (1, 0).GetLocal ());
          hwHeader.SetDstSeqno (temp2.GetSeqNo ());
          hwHeader.SetHopCount (temp2.GetHop () + 1);
          ...
          socket->SendTo (packet, 0, InetSocketAddress (destination, HW_PORT));
...
// broadcast the entire routing table for every periodicUpdateInterval
void
      ...
      for (std::map<Ipv4Address, RoutingTableEntry>::const_iterator i = allRoutes.begin (); i != allRoutes.end (); ++i)
        {
          ...
          if (i->second.GetHop () == 0)
            {
              ...
              hwHeader.SetDst (m_ipv4->GetAddress (1,0).GetLocal ());
              hwHeader.SetDstSeqno (i->second.GetSeqNo () + 2);
              hwHeader.SetHopCount (i->second.GetHop () + 1);
              m_routingTable.LookupRoute (m_ipv4->GetAddress (1,0).GetBroadcast (),ownEntry);
              ownEntry.SetSeqNo (hwHeader.GetDstSeqno ());
          ...
          NS_LOG_DEBUG ("Forwarding details are, Destination: " << hwHeader.GetDst ()
                                                                << ", SeqNo:" << hwHeader.GetDstSeqno ()
                                                                << ", HopCount:" << hwHeader.GetHopCount ()
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  NS_ASSERT (socket != 0);
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvHw,this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (Ipv4Address::GetAny (), HW_PORT));
      ...
      if (m_queue.Find (rt.GetDestination ()))
        {
          ...
          SendPacketFromQueue (rt.GetDestination (),route); // find the route, now send it
        }
    }
}

// Send packet from queue (unicast单播). 
void
...
// Get settlingTime for a destination, whether consider weights or not 
Time
...
// Merge periodic updates from a temperary adv_table to m_table. 
void
