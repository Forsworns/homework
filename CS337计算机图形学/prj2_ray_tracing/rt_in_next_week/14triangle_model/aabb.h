...
namespace yph {
	...
	public:
		...
		bool hit(ray<float> r,float tmin,float tmax) const {
			...
			for (int i = 0; i < 3;++i) {
				float t0 = ffmin((_min[i]-r.getOrigin()[i])/r.getDirection()[i], (_max[i] - r.getOrigin()[i]) / r.getDirection()[i]);
				float t1 = ffmax((_min[i] - r.getOrigin()[i]) / r.getDirection()[i], (_max[i] - r.getOrigin()[i]) / r.getDirection()[i]);
				tmin = ffmax(tmin, t0);
				tmax = ffmin(tmax, t1);
