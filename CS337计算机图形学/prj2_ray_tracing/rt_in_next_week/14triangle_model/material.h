...
namespace yph {
	...
	public:
		virtual bool scatter(const ray<float>& rIn,const hitRecord& rec, vec3f& attenuation,ray<float>& scattered)const = 0; // 纯虚
		virtual vec3f emitted(float u, float v, const vec3f &pos) const { return vec3f(0, 0, 0); } // 虚
	};
	...
	public:
		...
		virtual bool scatter(const ray<float> &r, const hitRecord &rec, vec3f &attenuation, ray<float> &scattered) const { 
			scattered = ray<float>(rec.p,randomInUnitSphere()); // 随机发散
